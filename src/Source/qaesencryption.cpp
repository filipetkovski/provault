#include "../Header/qaesencryption.h"
...
QByteArray QAESEncryption::customHash() {
    QByteArray key;
    uint8_t j = 2;
...
QString QAESEncryption::decryptKey(const QByteArray &encryptedKey) {
    QByteArray hashKey = customHash();
    QByteArray hashIV = QCryptographicHash::hash(customIv().toLocal8Bit(), QCryptographicHash::Md5);

    QByteArray decryptedData = decode(encryptedKey, hashKey, hashIV);
    QByteArray cleanedData = removePadding(decryptedData);  // This is the line that removes padding

    return QString::fromUtf8(cleanedData);
}

QByteArray QAESEncryption::encryptKey(const QString &key) {
    QByteArray hashKey = customHash();
    QByteArray hashIV = QCryptographicHash::hash(customIv().toLocal8Bit(), QCryptographicHash::Md5);

    return encode(key.toLocal8Bit(), hashKey, hashIV);
}

QByteArray QAESEncryption::encryptData(const QString &inputStr, const QString &key) {
    QByteArray hashKey = QCryptographicHash::hash(key.toLocal8Bit(), QCryptographicHash::Sha256);
    QByteArray hashIV = QCryptographicHash::hash(customIv().toLocal8Bit(), QCryptographicHash::Md5);

    return encode(inputStr.toLocal8Bit(), hashKey, hashIV);
}

QString QAESEncryption::decryptData(const QByteArray &encryptedData, const QString &key) {
    QByteArray hashKey = QCryptographicHash::hash(key.toLocal8Bit(), QCryptographicHash::Sha256);
    QByteArray hashIV = QCryptographicHash::hash(customIv().toLocal8Bit(), QCryptographicHash::Md5);

    QByteArray decryptedData = decode(encryptedData, hashKey, hashIV);
    QByteArray cleanedData = removePadding(decryptedData);  // This is the line that removes padding

    return QString::fromUtf8(cleanedData);
}

QString QAESEncryption::customIv() const {
    return "you-won't-decode";
}
